...
`ctx`作为上下文使用，Koa 将 node 的 r`equest`, `response`对象封装进一个单独对象。即`ctx.request 、 ctx.response`。Koa 内部又对一些常用的属性或者方法做了代理操作，使得我们可以直接通过 ctx 获取。比如，`ctx.request.ur`以写成`ctx.url`
...
1. 安装 koa 模板使用中间件`npm i koa-views`
